...
- **9 种控制流类型** — process、decision、join、delegate、end、错误路由、并行分叉、回环、收敛
- **三层分离** — 元数据、流程图、函数定义
- **子任务支持** — `main` + 命名子任务，通过 delegate 节点调用
- **错误处理** — 任何节点可定义 `error` 路由
...
| 类型 | 用途 | 写法 |
|------|------|------|
| `process` | 执行任务，然后进入下一节点 | `"next": ["reply"]` |
| `decision` | 条件分支 | `"branches": {"yes": "approve", "no": "reject"}` |
| `join` | 并行汇合，等待多个分支完成 | `"wait_for": ["step_a", "step_b"]` |
